...
if ( ! function_exists( 'bfi_wp_image_editor_check_notice' ) ) {
	function bfi_wp_image_editor_check_notice() {
	    printf( "<div class='error'>%s</div>",
	        esc_html__( "The server does not have ImageMagick or GD installed and/or enabled! Any of these libraries are required for WordPress to be able to resize images. Please contact your server administrator to enable this before continuing.", 'qalam' ) );
	}
}
...
if ( ! class_exists( 'BFI_Image_Editor_GD_1_3' ) ) {
	...
	class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
	    ...
	    public function rotate( $angle ) {
	        ...
	        return new WP_Error( 'image_rotate_error', esc_attr__( 'Image rotate failed.', 'qalam' ), $this->file );
	    }
	    ...
	    public function opacity( $opacity ) {
	        ...
	        return new WP_Error( 'image_opacity_error', esc_attr__('Image opacity change failed.', 'qalam' ), $this->file );
	    }
	    ...
	    public function colorize( $hexColor ) {
	        ...
	        return new WP_Error( 'image_colorize_error', esc_attr__( 'Image color change failed.', 'qalam' ), $this->file );
	    }
	    ...
	    public function grayscale() {
	        ...
	        return new WP_Error( 'image_grayscale_error', esc_attr__( 'Image grayscale failed.', 'qalam' ), $this->file );
	    }
	    ...
	    public function negate() {
	        ...
	        return new WP_Error( 'image_negate_error', esc_attr__( 'Image negate failed.', 'qalam' ), $this->file );
	    }
	}
}
...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        if( ! WP_Filesystem_Base::exists( $img_path ) || ! getimagesize( $img_path ) ) {
	            return $url;
	        }
	        ...
	        if ( WP_Filesystem_Base::exists( $destfilename ) && getimagesize( $destfilename ) ) {
	        } else {
	            // perform resizing and other filters
	            $editor = wp_get_image_editor( WP_Filesystem_Base::exists( $img_path ) );
	    ...
	    protected static function base_convert_arbitrary( $number, $fromBase, $toBase ) {
	        $digits = '0123456789abcdefghijqlmnopqrstuvwxyz';
