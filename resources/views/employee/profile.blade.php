...
<div class="profile-header mb-4">
    <div class="d-flex align-items-center gap-4">
        <div class="profile-avatar" id="staffAvatar">{{ substr(auth()->user()->name ?? 'S', 0, 1) }}{{ substr(strrchr(auth()->user()->name ?? '', ' ') ?: '', 1, 1) }}</div>
        <div>
            <h4 class="mb-1" id="staffName">{{ auth()->user()->name ?? 'Staff' }}</h4>
            <div class="mb-2 opacity-75" id="staffPosition">{{ auth()->user()->position ?? 'Staff Member' }}</div>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ auth()->user()->phone ?? 'Not set' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label" data-lang="wc-staff-position">Position</div>
                    <div class="info-value">{{ auth()->user()->position ?? 'Staff' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label" data-lang="wc-staff-department">Department</div>
                    <div class="info-value">{{ auth()->user()->department ?? 'General' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label" data-lang="wc-staff-employee-id">Employee ID</div>
                    <div class="info-value">{{ auth()->user()->employee_id ?? 'WC-' . auth()->user()->id }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label" data-lang="wc-staff-start-date">Start Date</div>
                    <div class="info-value">{{ auth()->user()->hire_date ? auth()->user()->hire_date->format('F j, Y') : '—' }}</div>
                </div>
            </div>
        </div>
