...
document.addEventListener('DOMContentLoaded', function(){
    ...
    async function apiPost(url, data = {}) {
        try {
            ...
            if (r.status === 401) { window.location.href = '/login'; return null; }
            return await r.json();
        } catch(e) { console.warn('API POST error:', url, e); return null; }
    }

    // ============ LOOKUPS ============
    const METHOD_MAP_TO_API = {
        card: 'card_terminal', cash: 'cash', transfer: 'bank_transfer', blik: 'blik',
        payu: 'card_terminal', p24: 'bank_transfer', paypal: 'bank_transfer',
        wise: 'bank_transfer', revolut: 'bank_transfer'
    };
    const METHOD_MAP_FROM_API = {
        card_terminal: 'card', cash: 'cash', bank_transfer: 'transfer', blik: 'blik'
    };
    const METHOD_ICONS = {
        ...
        card_terminal:'ri-bank-card-line text-primary', bank_transfer:'ri-bank-line text-info',
    ...
    const METHOD_LABELS = {
        ...
        card_terminal:'Card', bank_transfer:'Transfer',
    ...
    const STATUS_BADGES = {
        ...
        approved:'<span class="badge bg-success-subtle text-success">Completed</span>',
    ...
    function fmtPLN(n) {
        return Number(n || 0).toLocaleString('pl-PL', {minimumFractionDigits:2, maximumFractionDigits:2}) + ' PLN';
    }
    ...
    async function loadClients() {
        ...
        const newPayClient = document.getElementById('newPayClient');
        posClient.innerHTML = '<option value="">-- Select Client --</option>';
        newPayClient.innerHTML = '<option value="">-- Select --</option>';
        clientsList.forEach(c => {
            const name = (c.first_name || '') + ' ' + (c.last_name || '');
            posClient.innerHTML += `<option value="${c.id}">${name.trim()}</option>`;
            newPayClient.innerHTML += `<option value="${c.id}">${name.trim()}</option>`;
        });
    }

    // ============ LOAD CASES FOR CLIENT ============
    async function loadCasesForClient(clientId, selectEl) {
        selectEl.innerHTML = '<option value="">-- Loading... --</option>';
        if (!clientId) { selectEl.innerHTML = '<option value="">-- Select Case --</option>'; return; }
        const res = await apiGet('/clients/' + clientId + '/cases');
        selectEl.innerHTML = '<option value="">-- Select Case --</option>';
        if (!res || !res.success || !res.data) return;
        res.data.forEach(c => {
            selectEl.innerHTML += `<option value="${c.id}">${c.case_number || ('#' + c.id)}</option>`;
        });
    }
    ...
    posClient.addEventListener('change', function(){
        loadCasesForClient(this.value, posCase);
    });

    document.getElementById('newPayClient').addEventListener('change', function(){
        loadCasesForClient(this.value, document.getElementById('newPayCase'));
    });
    ...
    async function loadTransactions() {
        ...
        if (!res || !res.success) {
            document.getElementById('txnTableBody').innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">Failed to load transactions</td></tr>';
            return;
        }
        ...
        // Update stat cards
        document.getElementById('statTodayRev').textContent = fmtPLN(stats.today_total);
        document.getElementById('statTxnCount').textContent = stats.today_count || 0;
        const avgEl = document.getElementById('statTxnCount').closest('.card-body').querySelector('.fs-11');
        if (avgEl && stats.today_count > 0) {
            avgEl.textContent = 'Avg: ' + fmtPLN((stats.today_total || 0) / stats.today_count);
        } else if (avgEl) {
            avgEl.textContent = 'Avg: 0,00 PLN';
        }
        document.getElementById('statPending').textContent = stats.pending_count || 0;
        const pendAmtEl = document.getElementById('statPending').closest('.card-body').querySelector('.fs-11');
        if (pendAmtEl) pendAmtEl.textContent = fmtPLN(stats.pending_amount);
        document.getElementById('statMonthRev').textContent = fmtPLN(stats.total_received);
        const refEl = document.getElementById('statMonthRev').closest('.card-body').querySelector('.fs-11');
        if (refEl && stats.refunded_amount > 0) {
            refEl.innerHTML = '<i class="ri-refund-2-line"></i> Refunded: ' + fmtPLN(stats.refunded_amount);
        }
    ...
    document.getElementById('chargeBtn').addEventListener('click', async function(){
        ...
        const clientId = posClient.value;
        if(!clientId){ showToast('Select a client','warning'); return; }

        const client = clientsList.find(c => c.id == clientId);
        const clientName = client ? ((client.first_name || '') + ' ' + (client.last_name || '')).trim() : '';
        const caseId = posCase.value || '';
        const desc = document.getElementById('posDescription').value || 'POS Payment';
        const apiMethod = METHOD_MAP_TO_API[selectedMethod] || 'cash';

        this.disabled = true;
        this.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Processing...';

        const res = await apiPost('/pos/receive', {
        ...
        this.disabled = false;
        this.innerHTML = '<i class="ri-checkbox-circle-line me-2"></i>CHARGE <span id="chargeBtnAmount">0,00 PLN</span>';

        if (res && res.success) {
            showToast('Payment ' + fmtPLN(amount) + ' processed successfully', 'success');
            amountStr = '';
            updateDisplay();
            posClient.value = '';
            posCase.innerHTML = '<option value="">-- Select Case --</option>';
            ...
            const msg = (res && res.message) ? res.message : 'Failed to process payment';
    ...
    function renderTable(){
        ...
        if (!transactions.length) {
            ...
            document.getElementById('txnShowing').textContent = 'Showing 0 transactions';
            return;
        }
        let count = 0;
        transactions.forEach(t => {
            ...
            const method = METHOD_MAP_FROM_API[t.payment_method] || t.payment_method || 'cash';
            const amt = Number(t.amount || 0);
            const amtFormatted = fmtAmount(amt);
            const clientName = t.client_name || '';
            const caseDisplay = t.case_id ? ('#WC-' + String(t.case_id).padStart(4,'0')) : '';
            const dateDisplay = fmtDate(t.created_at);
            const operator = (t.received_by_user && t.received_by_user.name) ? t.received_by_user.name : '';
            const desc = t.service_description || t.notes || '';
            ...
            tbody.innerHTML += `
                <tr data-txn-id="${t.id}" data-client="${clientName}" data-client-id="${t.client_id||''}"
                    data-case="${caseDisplay}" data-case-id="${t.case_id||''}"
                    data-amount="${amtFormatted}" data-amount-raw="${amt}" data-method="${method}"
                    data-payment-method="${t.payment_method||''}" data-status="${status}"
                    data-date="${dateDisplay}" data-operator="${operator}" data-desc="${desc}"
                    data-receipt="${t.receipt_number||''}" data-notes="${(t.notes||'').replace(/"/g,'&quot;')}"
                    data-decision-notes="${(t.decision_notes||'').replace(/"/g,'&quot;')}">
                    <td><a href="#" class="fw-semibold text-body action-view">#${t.receipt_number || t.id}</a></td>
                    <td><a href="crm-client-invoices?client=${t.client_id||''}" class="text-body">${clientName}</a></td>
                    <td>${caseDisplay}</td>
                    <td class="fw-semibold ${status==='refunded'?'text-info':status==='rejected'?'text-danger':'text-success'}">${amtFormatted} PLN</td>
                    <td><i class="${METHOD_ICONS[method]||METHOD_ICONS[t.payment_method]||'ri-money-euro-circle-line text-muted'} me-1"></i>${METHOD_LABELS[method]||METHOD_LABELS[t.payment_method]||t.payment_method||''}</td>
                    <td>${STATUS_BADGES[status]||STATUS_BADGES.pending}</td>
                    <td class="text-muted fs-12">${dateDisplay}</td>
                    <td>${actions}</td>
                </tr>`;
        });
    ...
    // ============ DATE FILTERS ============
    document.getElementById('txnDateFrom').addEventListener('change', () => loadTransactions());
    document.getElementById('txnDateTo').addEventListener('change', () => loadTransactions());

    // ============ TABLE ACTIONS (event delegation) ============
    document.getElementById('txnTableBody').addEventListener('click', async function(e){
        ...
        if(btn.classList.contains('action-view')){
            document.getElementById('viewTxnId').textContent = '#' + (d.receipt || d.txnId);
            ...
            document.getElementById('viewTxnMethod').innerHTML = '<i class="' + (METHOD_ICONS[d.method]||'ri-money-euro-circle-line text-muted') + ' me-1"></i>' + (METHOD_LABELS[d.method]||d.method);
            document.getElementById('viewTxnStatus').innerHTML = STATUS_BADGES[d.status] || STATUS_BADGES.pending;
            document.getElementById('viewTxnOperator').textContent = d.operator;
            document.getElementById('viewTxnDesc').textContent = d.desc;
            document.getElementById('viewTxnModal').dataset.txnId = d.txnId;
            new bootstrap.Modal(document.getElementById('viewTxnModal')).show();
        }

        if(btn.classList.contains('action-receipt')){
            await showReceipt(d.txnId, d);
        }

        if(btn.classList.contains('action-approve')){
            btn.disabled = true;
            btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span>';
            const res = await apiPost('/pos/' + d.txnId + '/approve', { notes: 'Approved via POS' });
            if (res && res.success) {
                showToast('#' + (d.receipt || d.txnId) + ' approved', 'success');
                await loadTransactions();
            } else {
                showToast((res && res.message) || 'Failed to approve', 'danger');
                btn.disabled = false;
        ...
        if(btn.classList.contains('action-reject')){
            ...
            btn.disabled = true;
            btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span>';
            const res = await apiPost('/pos/' + d.txnId + '/reject', { reason: reason || 'Rejected via POS' });
            if (res && res.success) {
                showToast('#' + (d.receipt || d.txnId) + ' rejected', 'danger');
                await loadTransactions();
            } else {
                showToast((res && res.message) || 'Failed to reject', 'danger');
                btn.disabled = false;
        ...
        if(btn.classList.contains('action-refund')){
            document.getElementById('refundTxnId').textContent = '#' + (d.receipt || d.txnId);
            ...
            document.getElementById('refundModal').dataset.paymentMethod = d.paymentMethod || d.method;
            new bootstrap.Modal(document.getElementById('refundModal')).show();
        }
    });

    // ============ VIEW → RECEIPT ============
    document.getElementById('viewPrintReceiptBtn').addEventListener('click', async function(){
        const modal = bootstrap.Modal.getInstance(document.getElementById('viewTxnModal'));
        const txnId = document.getElementById('viewTxnModal').dataset.txnId;
        if(modal) modal.hide();
        setTimeout(async () => await showReceipt(txnId), 300);
    });

    // ============ RECEIPT ============
    async function showReceipt(txnId, fallbackData){
        ...
        const res = await apiGet('/pos/receipt/' + txnId);
        if (res && res.success && res.data) {
            const r = res.data;
            const method = METHOD_LABELS[METHOD_MAP_FROM_API[r.payment_method] || r.payment_method] || r.payment_method || '';
            body.innerHTML = `
                ...
                <div class="receipt-line"><span>Receipt:</span><span class="fw-semibold">#${r.receipt_number || r.id}</span></div>
                <div class="receipt-line"><span>Date:</span><span>${fmtDate(r.created_at)}</span></div>
                <div class="receipt-line"><span>Client:</span><span>${r.client_name || ''}</span></div>
                <div class="receipt-line"><span>Service:</span><span>${r.service_description || r.service_type || ''}</span></div>
                <div class="receipt-line"><span>Method:</span><span>${method}</span></div>
                ${r.terminal_transaction_id ? '<div class="receipt-line"><span>Terminal ID:</span><span>' + r.terminal_transaction_id + '</span></div>' : ''}
                <div class="receipt-line"><span>Net Amount:</span><span>${fmtPLN(r.net_amount)}</span></div>
                <div class="receipt-line"><span>Tax (${r.tax_rate||23}%):</span><span>${fmtPLN(r.tax_amount)}</span></div>
                <div class="receipt-line total"><span>TOTAL:</span><span>${fmtPLN(r.amount)}</span></div>
            ...
            body.innerHTML = `
                ...
                <div class="receipt-line"><span>Transaction:</span><span class="fw-semibold">#${d.receipt || d.txnId}</span></div>
                ...
                <div class="receipt-line"><span>Method:</span><span>${METHOD_LABELS[d.method]||d.method||''}</span></div>
            ...
            body.innerHTML = '<div class="text-center text-muted py-3">Receipt not available</div>';
        }
    }
    ...
    document.getElementById('confirmRefundBtn').addEventListener('click', async function(){
        ...
        const refundMethod = document.getElementById('refundModal').dataset.paymentMethod || 'cash';

        this.disabled = true;
        this.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Processing...';

        const res = await apiPost('/pos/' + txnId + '/refund', {
            refund_method: refundMethod
        });

        this.disabled = false;
        this.innerHTML = '<i class="ri-refund-2-line me-1"></i>Process Refund';

        if (res && res.success) {
            bootstrap.Modal.getInstance(document.getElementById('refundModal')).hide();
            showToast('Transaction refunded successfully', 'info');
            await loadTransactions();
        } else {
            showToast((res && res.message) || 'Failed to process refund', 'danger');
        }
    });

    // ============ NEW PAYMENT (header modal) ============
    document.getElementById('saveNewPayBtn').addEventListener('click', async function(){
        const clientId = document.getElementById('newPayClient').value;
        ...
        if(!clientId || !amount){ showToast('Fill client and amount','warning'); return; }

        const client = clientsList.find(c => c.id == clientId);
        const clientName = client ? ((client.first_name || '') + ' ' + (client.last_name || '')).trim() : '';
        const apiMethod = METHOD_MAP_TO_API[method] || 'cash';
        ...
        const res = await apiPost('/pos/receive', {
            ...
            client_passport: '',
            service_type: 'consultation',
            service_description: desc || 'Quick payment',
            payment_method: apiMethod,
            amount: amount,
            currency: 'PLN',
            tax_rate: 23,
        ...
        this.disabled = false;
        this.innerHTML = '<i class="ri-checkbox-circle-line me-1"></i>Process Payment';

        if (res && res.success) {
            bootstrap.Modal.getInstance(document.getElementById('newPaymentModal')).hide();
            showToast('Payment processed: ' + fmtPLN(amount), 'success');
            document.getElementById('newPayClient').value = '';
            document.getElementById('newPayCase').innerHTML = '<option value="">-- Select --</option>';
            document.getElementById('newPayAmount').value = '';
            document.getElementById('newPayDesc').value = '';
            await loadTransactions();
        } else {
            showToast((res && res.message) || 'Failed to process payment', 'danger');
        }
    });

    // ============ EXPORT CSV ============
    document.getElementById('exportCsvBtn').addEventListener('click', function(){
        let csv = 'ID,Receipt,Client,Case,Amount,Currency,Tax,Net,Method,Status,Date,Description\n';
        transactions.forEach(t => {
            const method = METHOD_LABELS[METHOD_MAP_FROM_API[t.payment_method] || t.payment_method] || t.payment_method || '';
            const status = normalizeStatus(t.status);
            csv += `${t.id},"${t.receipt_number||''}","${t.client_name||''}","${t.case_id||''}",${t.amount||0},${t.currency||'PLN'},${t.tax_amount||0},${t.net_amount||0},"${method}",${status},"${fmtDate(t.created_at)}","${(t.service_description||'').replace(/"/g,'""')}"\n`;
        });
    ...
    document.getElementById('dailyReportBtn').addEventListener('click', async function(){
        ...
        const res = await apiGet('/pos/report/daily?date=' + dateInput);
        if (res && res.success && res.data) {
            const r = res.data;
            document.getElementById('reportRevenue').textContent = fmtPLN(r.total_revenue || r.total || 0);
            document.getElementById('reportTxns').textContent = r.transaction_count || r.count || 0;
            document.getElementById('reportPending').textContent = r.pending_count || 0;
            document.getElementById('reportRefunds').textContent = fmtPLN(r.refund_total || r.refunded_amount || 0);

            // Update method breakdown table if available
            if (r.by_method && Array.isArray(r.by_method)) {
                const methodIcons = {
                    card_terminal: 'ri-bank-card-line me-1 text-primary',
                    cash: 'ri-money-euro-circle-line me-1 text-success',
                    bank_transfer: 'ri-bank-line me-1 text-info',
                    blik: 'ri-smartphone-line me-1 text-danger'
                };
                const methodColors = {
                    card_terminal: 'primary', cash: 'success', bank_transfer: 'info', blik: 'danger'
                };
                const totalAmt = r.by_method.reduce((s,m) => s + Number(m.total || m.amount || 0), 0);
                const tbody = document.querySelector('#dailyReportModal .table tbody');
                if (tbody) {
                    tbody.innerHTML = '';
                    r.by_method.forEach(m => {
                        const icon = methodIcons[m.method || m.payment_method] || 'ri-money-euro-circle-line me-1 text-muted';
                        const color = methodColors[m.method || m.payment_method] || 'secondary';
                        const label = METHOD_LABELS[METHOD_MAP_FROM_API[m.method || m.payment_method] || m.method || m.payment_method] || m.method || '';
                        const share = totalAmt > 0 ? Math.round((Number(m.total || m.amount || 0) / totalAmt) * 100) : 0;
                        tbody.innerHTML += `<tr>
                            <td><i class="${icon}"></i>${label}</td>
                            <td>${m.count || 0}</td>
                            <td>${fmtPLN(m.total || m.amount || 0)}</td>
                            <td><div class="progress" style="height:6px"><div class="progress-bar bg-${color}" style="width:${share}%"></div></div></td>
                        </tr>`;
                    });
                }
            }

            // Update top clients if available
            if (r.top_clients && Array.isArray(r.top_clients)) {
                const clientBadgesEl = document.querySelector('#dailyReportModal .d-flex.gap-2.flex-wrap');
                if (clientBadgesEl) {
                    const colors = ['primary', 'success', 'info', 'warning', 'danger'];
                    clientBadgesEl.innerHTML = '';
                    r.top_clients.forEach((c, i) => {
                        const col = colors[i % colors.length];
                        clientBadgesEl.innerHTML += `<span class="badge bg-${col}-subtle text-${col} p-2">${c.name || c.client_name || ''} — ${fmtPLN(c.total || c.amount || 0)}</span>`;
                    });
                }
            }
        }
